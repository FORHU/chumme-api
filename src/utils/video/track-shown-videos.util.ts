...
export async function getShownVideos(userId: string): Promise<string[]> {
  try {
    ...
    logger.error(
      `[VIDEO-HISTORY] Error getting shown videos: ${error?.message}`,
    );
    return [];
  }
}
 ...
 */
export async function addShownVideo(
  userId: string,
  videoId: string,
): Promise<void> {
  try {
    ...
    logger.info(
      `[VIDEO-HISTORY] Added video ${videoId} to user ${userId} history (${recentVideos.length} total)`,
    );
  } catch (error: any) {
...
export async function clearShownVideos(userId: string): Promise<void> {
  try {
    ...
    logger.error(
      `[VIDEO-HISTORY] Error clearing shown videos: ${error?.message}`,
    );
  }
}
...
export async function filterShownVideos(
  ...
  try {
    ...
    logger.error(
      `[VIDEO-HISTORY] Error filtering shown videos: ${error?.message}`,
    );
