...
export async function fetchVideoRecommendation(
    ...
    try {
        ...
        logger.info(
            `[VIDEO-RECOMMENDATION] Starting video recommendation flow`
        );
        ...
        if (
            ...
            const requestedArtistIds = multipleArtistsResult.artists
                .map(
                    (artistName) =>
                        allArtists.find((a) => a.name === artistName)?.id
                )
        ...
        if (!videos || videos.length === 0) {
            logger.warn(
                `[VIDEO-RECOMMENDATION] No videos found matching criteria`
            );
            const metadata: any = {};
