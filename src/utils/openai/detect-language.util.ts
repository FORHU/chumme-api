 ...
 */
export async function detectLanguage(userInput: string): Promise<string | null> {
    try {
        // Quick check: if all ASCII and common English words, skip AI call
        const hasNonEnglishChars = /[^\x00-\x7F]/.test(userInput);
        const commonEnglishPattern = /\b(i|am|is|are|was|were|the|a|an|my|your|give|show|play|want|like|feel|sad|happy)\b/i;
