...
export default class RelayManager {
  ...
  static async getNextAutoSingerId(
    studioId: string,
    lineIndex: number,
  ): Promise<{ nextSingerId: string | null; targetRoleIndex: number | null }> {
    ...
    if (!shouldSwitch) return { nextSingerId: null, targetRoleIndex };
    ...
    if (singers.length === 0) return { nextSingerId: null, targetRoleIndex };
    ...
    // Only switch if it's actually a new person
    if (nextSinger && nextSinger.userId !== currentSingerId) {
      return { nextSingerId: nextSinger.userId, targetRoleIndex };
    }

    return { nextSingerId: null, targetRoleIndex };
  }
}
