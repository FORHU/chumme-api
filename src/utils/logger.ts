...
const chatLogger = winston.createLogger({
    ...
    transports: [
        new winston.transports.File({ filename: "chat-send.log"})
    ]
})
...
function createModuleLogger(){
    return {
        ...
        chat_response: (msg: string, ...meta: any[]) => chatLogger.info(`${msg}`, ...meta),
        chat_error: (msg: string, ...meta: any[]) => chatLogger.error(`${msg}`, ...meta),
