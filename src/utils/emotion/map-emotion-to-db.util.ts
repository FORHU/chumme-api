...
export async function mapEmotionToDatabase(
  ...
  try {
    ...
    logger.info(
      `[EMOTION-MAPPING] No exact match for "${detectedEmotion}", finding closest match...`,
    );
...
`;
    ...
    if (!normalizedAvailable.includes(mappedEmotion)) {
      logger.warn(
        `[EMOTION-MAPPING] AI returned invalid emotion "${mappedEmotion}", defaulting to "neutral"`,
      );
      return {
    ...
    logger.info(
      `[EMOTION-MAPPING] Successfully mapped "${detectedEmotion}" → "${mappedEmotion}"`,
    );
