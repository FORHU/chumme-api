...
export class VideoPostListener {
  ...
  async connect(): Promise<void> {
    try {
      console.log("Connecting to RabbitMQ for tiktok post events...");
      ...
      this.channel.on("error", (err: any) => {
        console.error("RabbitMQ channel error (TikTok):", err);
        this.isConnected = false;
      });

      this.channel.on("close", () => {
        console.log("RabbitMQ channel closed (TikTok). Reconnecting...");
        this.isConnected = false;
        setTimeout(
      ...
      console.error("Failed to connect to RabbitMQ (TikTok):", error);
      this.isConnected = false;
      throw error;
    }
  }
