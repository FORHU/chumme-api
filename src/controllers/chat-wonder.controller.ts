...
export default class ChatWonderCtrl {
  ...
  static async streamChat(req: Request, res: Response, next: NextFunction) {
      ...
      try {
        ...
        // Generate session ID
        const chatSessionId = (await ChatWonderSvc.generateChatSessionId(userId)) ?? '';
        ...
        await streamChat(chummePrompt, chatSessionId, {
          ...
          onComplete: async () => {
            try {
              logger.info('[CHAT-WONDER-STREAM] Stream completed, saving AI response');
              ...
              logger.error(`[CHAT-WONDER-STREAM] Error saving response: ${err.message}`);
